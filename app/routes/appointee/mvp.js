...
router.post('/appointee/mvp/', function (req, res) {
  ...
  req.session.data['appointee-correspondence-country'] = 'England';
  ...
  req.session.data['appointee-residential-country'] = 'England';
  ...
  req.session.data['appointee-residential-address'] = req.session.data['appointee-residential-line-1'] + ', ' +  req.session.data['appointee-residential-line-2'] + ', ' +  req.session.data['appointee-residential-line-3'] + ', ' +  req.session.data['appointee-residential-postcode'] + ', ' +  req.session.data['appointee-residential-country'];
    // End of information
  res.redirect('/appointee/mvp/searchlight/');
});

router.post('/appointee/mvp/searchlight/', function (req, res) {
  if (req.session.data['journey-choice'] == 'authorise') {
    res.redirect('/appointee/mvp/searchlight/customer/proposed');
  } else { 
    res.redirect('/appointee/mvp/searchlight/customer/');
  }
});

router.post('/appointee/mvp/searchlight/customer/', function (req, res) {
  res.redirect('/appointee/mvp/propose/details');
});

router.post('/appointee/mvp/propose/', function (req, res) {
  if (req.session.data['appointee-nino-choice'] == 'yes') {
    res.redirect('/appointee/mvp/propose/nino');
  } else {
    res.redirect('/appointee/mvp/propose/details');
  }
});

router.post('/appointee/mvp/propose/nino', function (req, res) {
  res.redirect('/appointee/mvp/propose/summary');
});

router.post('/appointee/mvp/propose/details', function (req, res) {
  if (req.session.data['journey-choice'] == 'propose') {
    res.redirect('/appointee/mvp/propose/summary');
  } else {
    res.redirect('/appointee/mvp/service/no-match-found');
  }
});

//router.post('/appointee/mvp/propose/appointee-correct', function (req, res) {
//  if (req.session.data.appointeeCorrect == 'appointee-correct-yes') {
//    req.session.data.relationshipStatus = 'asserted';
//    res.redirect('/appointee/mvp/propose/confirmation');
//  } else {
//    req.session.data.authorisedUser = null;
//    res.redirect('/appointee/mvp/propose/');
//  }
//});

router.post('/appointee/mvp/propose/summary', function (req, res) {
  req.session.data.relationshipStatus = 'asserted';
  res.redirect('/appointee/mvp/searchlight/customer/proposed');
});

router.post('/appointee/mvp/searchlight/customer/proposed', function (req, res) {
  req.session.data['journey-choice'] = 'authorise';
  res.redirect('/appointee/mvp/authorise/');
});
...
router.post('/appointee/mvp/authorise/', function (req, res) {
  res.redirect('/appointee/mvp/authorise/decision-date');
});

router.post('/appointee/mvp/authorise/decision-date', function (req, res) {
  if (req.session.data['decision-date'] == 'today') {
    if (req.session.data['verified-appointeeship'] == 'authorised') {
      ...
      res.redirect('/appointee/mvp/authorise/authorised/review-date');
    } else {
      req.session.data.relationshipStatus = 'not-authorised';
      res.redirect('/appointee/mvp/authorise/not-authorised/summary');
    }
  } else {
    res.redirect('/appointee/mvp/authorise/decision-date-manual');
  }
});

router.post('/appointee/mvp/authorise/decision-date-manual', function (req, res) {
  if (req.session.data['verified-appointeeship'] == 'authorised') {
    req.session.data.relationshipStatus = 'authorised';
    res.redirect('/appointee/mvp/authorise/authorised/review-date');
  } else {
...
router.post('/appointee/mvp/authorise/authorised/', function (req, res) {
  if (req.session.data['verified-date'] == 'today') {
    res.redirect('/appointee/mvp/authorise/authorised/review-date');
  } else {
    res.redirect('/appointee/mvp/authorise/authorised/date');
  }
});

router.post('/appointee/mvp/authorise/authorised/review-date', function (req, res) {
  if (req.session.data['review-date'] == 'custom') {
    res.redirect('/appointee/mvp/authorise/authorised/custom-review-date');
  } else {
    ...
    res.redirect('/appointee/mvp/authorise/authorised/summary');
  }
});

router.post('/appointee/mvp/authorise/authorised/date', function (req, res) {
  res.redirect('/appointee/mvp/authorise/authorised/review-date');
});

router.post('/appointee/mvp/authorise/authorised/custom-review-date', function (req, res) {
  res.redirect('/appointee/mvp/authorise/authorised/summary');
});

router.post('/appointee/mvp/authorise/authorised/summary', function (req, res) {
  res.redirect('/appointee/mvp/searchlight/customer/authorised');
});

// Declined

router.post('/appointee/mvp/authorise/not-authorised/', function (req, res) {
  if (req.session.data['declined-date'] == 'today') {
    res.redirect('/appointee/mvp/authorise/not-authorised/summary');
  } else {
    res.redirect('/appointee/mvp/authorise/not-authorised/date');
  }
});

router.post('/appointee/mvp/authorise/not-authorised/reason', function (req, res) {
  res.redirect('/appointee/mvp/authorise/not-authorised/summary');
});

router.post('/appointee/mvp/authorise/not-authorised/date', function (req, res) {
  res.redirect('/appointee/mvp/authorise/not-authorised/summary');
});

router.post('/appointee/mvp/authorise/not-authorised/summary', function (req, res) {
  res.redirect('/appointee/mvp/searchlight/customer/not-authorised');
});

// Change

router.post('/appointee/mvp/propose/change/details', function (req, res) {
  // changed values
  req.session.data['appointee-dob'] = '25 March 1993';
  res.redirect('/appointee/mvp/propose/change/summary');
});

router.post('/appointee/mvp/propose/change/summary', function (req, res) {
  req.session.data.relationshipStatus = 'asserted';
  res.redirect('/appointee/mvp/searchlight/customer/proposed');
});
